...
REF_FASTA="/rd/caiya/hg38.fa"
SAMPLES="W1-C18-CD1 W1-C18-Ch2 W2-C18-SDS1 W1-C18-CD2 W2-C18-651 W2-C18-SDS2 W1-C18-Ch1 W2-C18-652"

cd ${WORK_DIR}
mkdir -p ./tmp_sort_dir

for file in ${RAW_DIR}/*/RawData/*/*.fq.gz; do
...
for r in ${SAMPLES}; do
    ...
    mosdepth_prefix="${r}.F904"
    /rd2/wangwbx/software/mosdepth/mosdepth -n -t 16 -b 100000 $mosdepth_prefix ${r}.pe.F904.s.bam
    ...
    echo -e "${r}\t${over0_count}\t${total_count}" >> ${DATE}.coverage.count

    samtools view -@ 8 ${r}.pe.F904.s.bam | awk -v id="$r" 'BEGIN{all_count=0; split_count=0} {if(int($2/64)%2 == 1){all_count++; if($7 != "=" || $9 > 1000 || $9 < -1000){split_count++}}} END{print id"\t"split_count"\t"all_count}' >> ${DATE}.discordant.count
    ...
    pigz -dc $fastq_1 | awk 'BEGIN{all_base = 0}{if(NR % 4 == 2){all_base = all_base + length($0)}}END{print "'$r'""\tread_1\t"all_base}' >> ${DATE}.total_base.count &
    p1=$!
    pigz -dc $fastq_2 | awk 'BEGIN{all_base = 0}{if(NR % 4 == 2){all_base = all_base + length($0)}}END{print "'$r'""\tread_2\t"all_base}' >> ${DATE}.total_base.count &
    p2=$!
